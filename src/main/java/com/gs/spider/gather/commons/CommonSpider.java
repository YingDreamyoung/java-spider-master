...
//        }
            ...
            if ((!info.isGatherFirstPage() && startPage) || (info.isGatherFirstPage() && startPage)) {
                List<String> links = null;
                if (StringUtils.isNotBlank(info.getUrlReg())&& StringUtils.isNotBlank(info.getDetailUrlXPath())) {//url正则式不为空 && 精确获取详情页 url
                    links = page.getHtml().xpath(info.getDetailUrlXPath()).links().regex(info.getUrlReg()).all().stream()
                            .map(s -> {
...
//                    }
//                }
                log.debug("获取页面的link，{}", page.getUrl().get());
                log.debug("获取页面的links，{}", links.toString());
                links = links.stream().filter(s -> StringUtils.isNotBlank(s)).collect(Collectors.toList());
                String urlMd5="";
                if(links!=null && !links.isEmpty()){
                    if(links.size()>info.getMaxPageGather())
                        links=links.subList(0,info.getMaxPageGather());
                    for (String url:links) {
                        ...
                        if(StringUtils.isNotBlank(redisUrl)){
                            //如果 redis 中存在说明已存储
                            //不会再爬取
                            log.warn("redis中存在: {}"+StringUtils.isNotBlank(redisUrl));
                            continue;
                        }
...
//            page.putField("content", HTML_PREFIX + content + HTML_SUFFIX);
            ...
            //抽取作者
            String author = null;
            if (!StringUtils.isBlank(info.getAuthorXPath())) {//提取网页作者
                author = page.getHtml().xpath(info.getAuthorXPath()).get();
            } else if (!StringUtils.isBlank(info.getAuthorReg())) {
                author = page.getHtml().regex(info.getAuthorReg()).get();
            } else {//如果不写默认是author
                author = "no author";
            }
            page.putField("author", author);
            if (info.isNeedAuthor() && StringUtils.isBlank(author)) {//if the author is blank ,skip it!
                page.setSkip(true);
                return;
            }
            ...
            //抽取发布时间
/*
            TODO 发布时间代做优化，如果没有合乎要求的时间，选取当前时间
*/
